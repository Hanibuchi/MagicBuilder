...
public class EquippedSpellModel : MonoBehaviour, ISpellHoldInfoObserver, IEquippedSpellsObserver
{
    ...
    // EquippedSpellModel.cs の末尾に追記

    [Header("--- EquippedSpellModel テスト設定 ---")]
    [SerializeField] private SpellType test_spellTypeToAdd = SpellType.ExampleSpell;
    [SerializeField] private int test_slotIndex = 0;
    [SerializeField] private int test_capacityIncreaseAmount = 1;


    // ----------------------------------------------------------------------------------
    // デバッグ/テスト用メソッド (Unity Buttonから呼び出し可能)
    // ----------------------------------------------------------------------------------

    /// <summary>
    /// 全ての呪文の現在のステータス (所持数、利用可能数、アンロック状態) をログに出力します。
    /// </summary>
    public void Test_LogAllStatuses()
    {
        Debug.Log("--- EquippedSpellModel: 全ステータスログ ---");
        var statuses = GetAllSpellStatuses().OrderBy(s => s.Type.ToString());

        foreach (var status in statuses)
        {
            if (status.TotalCount > 0 || status.EquippedCount > 0 || status.IsUnlocked)
            {
                Debug.Log($"[Status] Type: {status.Type}\n" +
                          $"  - Unlocked: {status.IsUnlocked}\n" +
                          $"  - Total Count: {status.TotalCount}\n" +
                          $"  - Equipped Count: {status.EquippedCount}\n" +
                          $"  - Available Count: {status.AvailableCount}");
            }
        }
    }

    /// <summary>
    /// 指定したインデックスに、指定したSpellTypeの呪文を装備します。
    /// （EquippedSpellManager.SetSpell を呼び出す）
    /// </summary>
    public void Test_SetSpellToSlot()
    {
        SpellHoldStatus targetStatus = _allSpellStatuses.GetValueOrDefault(test_spellTypeToAdd);

        if (targetStatus == null || targetStatus.AvailableCount <= 0)
        {
            Debug.LogError($"[Test Fail] SpellType.{test_spellTypeToAdd} は利用可能数 (AvailableCount) が0のため装備できません。");
            return;
        }

        // SpellTypeからSpellBaseアセットを取得
        // ※このテストコードでは、SpellDatabase.Instanceが存在し、GetSpellAssetが有効なSpellBaseアセットを返す前提です。
        SpellBase spellAsset = SpellDatabase.Instance?.GetSpellAsset(test_spellTypeToAdd);

        if (spellAsset == null)
        {
            Debug.LogError($"[Test Fail] SpellType.{test_spellTypeToAdd} に対応する SpellAsset が見つかりません。");
            return;
        }

        // EquippedSpellManagerを通して呪文をセット
        bool success = EquippedSpellManager.Instance.SetSpell(test_slotIndex, spellAsset);

        if (success)
        {
            Debug.Log($"<color=lime>[Test Success]</color> Slot {test_slotIndex} に SpellType.{test_spellTypeToAdd} を装備しました。");
        }
        // SetSpellが失敗した場合のログは EquippedSpellManager 側で出力されます。

        Test_LogEquippedSpells();
    }

    /// <summary>
    /// 指定したインデックスの呪文をスロットから削除します。
    /// （EquippedSpellManager.RemoveSpell を呼び出す）
    /// </summary>
    public void Test_RemoveSpellFromSlot()
    {
        if (test_slotIndex < 0 || test_slotIndex >= EquippedSpellManager.Instance.GetMaxCapacity())
        {
            Debug.LogError($"[Test Fail] 無効なスロットインデックス: {test_slotIndex}");
            return;
        }

        // EquippedSpellManagerを通して呪文を削除
        EquippedSpellManager.Instance.RemoveSpell(test_slotIndex);

        Debug.Log($"<color=yellow>[Test Success]</color> Slot {test_slotIndex} の呪文を削除しました。");
    ...
    /// <summary>
    /// 持ち込みスロットの最大容量を増やします。
    /// （EquippedSpellManager.IncreaseCapacity を呼び出す）
    /// </summary>
    public void Test_IncreaseCapacity()
    {
        if (test_capacityIncreaseAmount <= 0)
        {
            Debug.LogWarning("[Test Warning] 容量増加量は1以上を指定してください。");
            return;
        }

        EquippedSpellManager.Instance.IncreaseCapacity(test_capacityIncreaseAmount);

        Debug.Log($"<color=cyan>[Test Success]</color> 持ち込み容量を {test_capacityIncreaseAmount} 増やしました。新しい最大容量: {GetMaxCapacity()}");
    }

    /// <summary>
    /// 現在の装備スロットの内容をログに出力します。
    /// </summary>
    public void Test_LogEquippedSpells()
    {
        var equipped = GetEquippedSpells();
        string log = "--- EquippedSpells Log (Max: " + GetMaxCapacity() + ") ---\n";

        for (int i = 0; i < equipped.Count; i++)
        {
            string spellName = equipped[i] == null
                ? "[NULL/EMPTY]"
                : SpellDatabase.Instance.GetSpellType(equipped[i]).ToString(); // SpellDatabaseを使用する代わりにManagerのメソッドを利用

            log += $"Slot [{i}]: {spellName}\n";
        }
